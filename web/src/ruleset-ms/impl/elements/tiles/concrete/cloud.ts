...
import type { MsTileLeaveBehaviorContext } from "@ruleset-ms/impl/elements/tiles/families/leave";

export function createMsCloudTileBehavior(tileId: number): TileBehavior<number, number> | undefined {
  if (tileId !== MS_TILE.Cloud) {
    return undefined;
  }
  return createTileBehavior({
    "complete-exit": (context) => {
      const behaviorContext = context as MsTileLeaveBehaviorContext;
