...
describe("MS engine regressions", () => {
  ...
  it("uses same-layer trap connections when Chip presses a brown button on z2", () => {
    ...
    const buttonPos = pos(13, 15);
    const lowerTrapPos = pos(12, 13);
    const upperTrapPos = pos(12, 14);
    lower[buttonPos]!.top.id = MS_TILE.Button_Brown;
    lower[lowerTrapPos]!.top.id = msCreatureTile(MS_TILE.Bug, MS_DIRECTION.south);
    lower[lowerTrapPos]!.bottom.id = MS_TILE.Beartrap;
    upper[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.west);
    upper[buttonPos]!.top.id = MS_TILE.Button_Brown;
    upper[upperTrapPos]!.top.id = msCreatureTile(MS_TILE.Fireball, MS_DIRECTION.south);
    upper[upperTrapPos]!.bottom.id = MS_TILE.Beartrap;
    ...
    const next = advanceMsInteractiveSession(
      createMsInteractiveSession(
        ...
        createLevel({
          cells: lower,
          creaturePositions: [lowerTrapPos],
          layers: [
            {
              z: 1,
              cells: lower,
              traps: [{ from: buttonPos, to: lowerTrapPos, fromZ: 1, toZ: 1 }],
              cloners: [],
              creaturePositions: [lowerTrapPos],
              hintText: "",
            },
            {
              z: 2,
              cells: upper,
              traps: [{ from: buttonPos, to: upperTrapPos, fromZ: 2, toZ: 2 }],
              cloners: [],
              creaturePositions: [chipPos, upperTrapPos],
              hintText: "",
            },
          ],
        }),
    ...
    expect(next.state.internal.chipZ).toBe(2);
    expect(next.state.internal.creatures.find((creature) => creature.pos === upperTrapPos && creature.z === 2)?.released).toBe(true);
    expect(next.state.internal.creatures.find((creature) => creature.pos === lowerTrapPos && creature.z === 1)?.released).toBe(false);
  });

  it("uses the active z-layer when Chip teleports on z2", () => {
    const lower = createEmptyCells();
    const upper = createEmptyCellsAtZ(2);
    const chipPos = pos(13, 9);
    const entryTeleportPos = pos(14, 9);
    const lowerExitPos = pos(10, 9);
    const upperExitPos = pos(11, 9);
    lower[entryTeleportPos]!.top.id = MS_TILE.Teleport;
    lower[lowerExitPos]!.top.id = MS_TILE.Teleport;
    upper[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east);
    upper[entryTeleportPos]!.top.id = MS_TILE.Teleport;
    upper[upperExitPos]!.top.id = MS_TILE.Teleport;
    ...
    expect(next.state.internal.chipZ).toBe(2);
    expect(next.state.internal.chipPos).toBe(upperExitPos);
    expect(next.state.engine.map.cells[upperExitPos]?.top.id).toBe(msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east));
  });
  ...
  it("keeps teeth targeting Chip by x/y even when they are on a different z-layer", () => {
    ...
    upper[teethPos]!.top.id = msCreatureTile(MS_TILE.Teeth, MS_DIRECTION.east);
    upper[wallPos]!.top.id = MS_TILE.Wall;
    ...
    const teeth = session.state.engine.actors.find((actor) => actor.id === MS_TILE.Teeth && actor.position.z === 2);
  ...
  it("drops a tracked block from unsupported air into water on the first non-chip floor tick", () => {
    const lower = createEmptyCells();
    const upper = createEmptyCellsAtZ(2);
    const blockPos = pos(11, 8);
    lower[blockPos]!.top.id = MS_TILE.Water;
    ...
    expect(session.state.engine.map.layers?.[0]?.cells[blockPos]?.top.id).toBe(MS_TILE.Dirt);
    expect(session.state.engine.map.layers?.[1]?.cells[blockPos]?.top.id).toBe(MS_TILE.Air);
    expect(session.state.internal.blocks.find((block) => block.pos === blockPos && (block.z ?? 1) === 2)?.hidden).toBe(true);
    expect((session.state.engine.soundEffects & (1 << MS_SOUND.WaterSplash)) !== 0).toBe(true);
  });

  it("does not drop a tracked block from air when another non-player below supports it", () => {
    const lower = createEmptyCells();
    const upper = createEmptyCellsAtZ(2);
    const blockPos = pos(12, 8);
    lower[blockPos]!.top.id = MS_TILE.Block_Static;
    upper[blockPos]!.top.id = MS_TILE.Block_Static;
    upper[blockPos]!.bottom.id = MS_TILE.Air;
  ...
  it("drops a non-player from unsupported air onto Chip and collides on landing", () => {
    ...
    lower[sharedPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.south);
    ...
    let session = createMsInteractiveSession(
      ...
      createLevel({
        cells: lower,
        creaturePositions: [sharedPos],
        layers: [
          { z: 1, cells: lower, traps: [], cloners: [], creaturePositions: [sharedPos], hintText: "" },
  ...
  it("does not arm ice movement when Chip falls from unsupported air onto ice", () => {
    ...
    const upper = createEmptyCellsAtZ(2);
    const chipPos = pos(15, 8);
    lower[chipPos]!.top.id = MS_TILE.Ice;
    upper[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.south);
    upper[chipPos]!.bottom.id = MS_TILE.Air;
  ...
  it("arms force-floor movement when Chip falls from unsupported air onto a force floor", () => {
    const lower = createEmptyCells();
    const upper = createEmptyCellsAtZ(2);
    const chipPos = pos(16, 8);
    lower[chipPos]!.top.id = MS_TILE.Slide_South;
    upper[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.west);
    upper[chipPos]!.bottom.id = MS_TILE.Air;
  ...
  it("bombs Chip when he falls from unsupported air onto a bomb", () => {
    const lower = createEmptyCells();
    const upper = createEmptyCellsAtZ(2);
    const chipPos = pos(16, 9);
    lower[chipPos]!.top.id = MS_TILE.Bomb;
    upper[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.west);
    upper[chipPos]!.bottom.id = MS_TILE.Air;
    ...
    expect(session.state.internal.chipZ).toBe(1);
    expect(session.state.internal.chipPos).toBe(chipPos);
    expect(session.state.internal.chipStatus).toBe("bombed");
    expect((session.state.engine.soundEffects & (1 << MS_SOUND.BombExplodes)) !== 0).toBe(true);
    expect(session.state.engine.map.cells[chipPos]?.top.id).toBe(MS_TILE.Bombed_Chip);
  });

  it("keeps a pickup under a block when the block falls from unsupported air", () => {
    const lower = createEmptyCells();
    const upper = createEmptyCellsAtZ(2);
    const blockPos = pos(17, 8);
    lower[blockPos]!.top.id = MS_TILE.Key_Red;
    upper[blockPos]!.top.id = MS_TILE.Block_Static;
    upper[blockPos]!.bottom.id = MS_TILE.Air;
    ...
    expect(block?.pos).toBe(blockPos);
    expect(session.state.engine.map.cells[blockPos]?.top.id).toBe(MS_TILE.Block_Static);
    expect(session.state.engine.map.cells[blockPos]?.bottom.id).toBe(MS_TILE.Key_Red);
  });
  ...
  it("snatches an adjacent monster into a carried pet carrier", () => {
    ...
    const eastPos = pos(9, 10);
    cells[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east);
    cells[eastPos]!.top.id = msCreatureTile(MS_TILE.Bug, MS_DIRECTION.south);
    ...
    addCarriedMsPetCarrier(session);
  ...
  it("snatches an adjacent block off a clone machine into a carried pet carrier", () => {
    ...
    expect(session.state.engine.map.cells[eastPos]?.top.id).toBe(MS_TILE.CloneMachine);
    expect(session.state.engine.map.cells[eastPos]?.bottom.id).toBe(MS_TILE.Empty);
    expect(session.state.internal.blocks.find((block) => block.pos === eastPos && !block.hidden)).toBeUndefined();
    expect(carried).toMatchObject({
      family: "pet-carrier",
      petCarrierState: {
        occupant: {
  ...
  it("does not snatch through a thin wall on Chip's tile", () => {
    ...
    expect(session.state.engine.map.cells[eastPos]?.top.id).toBe(msCreatureTile(MS_TILE.Bug, MS_DIRECTION.south));
  ...
  it("does not snatch adjacent special items", () => {
    ...
    const eastPos = pos(9, 10);
    cells[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east);
    cells[eastPos]!.top.id = MS_TILE.Sandbag;
    ...
    expect(session.state.engine.map.cells[eastPos]?.top.id).toBe(MS_TILE.Sandbag);
    expect(session.state.internal.portableTools.portableItems.find((item) => item.state.mode === "carried")).toMatchObject({
      family: "pet-carrier",
      petCarrierState: {
        occupant: null,
  ...
  it("throws a carried bowling ball into an empty forward cell through the portable action seam", () => {
    ...
    const thrownItem = session.state.internal.portableTools.portableItems.find((item) => item.state.mode === "attached");
    const bowlingBall = session.state.internal.creatures.find((creature) => creature.id === MS_TILE.BowlingBall && !creature.hidden);
    const runtimeEntry = bowlingBall ? findStatefulActorRuntime(msStatefulActorsForTest(session.state), bowlingBall.serial) : undefined;
    ...
    expect(session.state.engine.map.cells[eastPos]?.top.id).toBe(msCreatureTile(MS_TILE.BowlingBall, MS_DIRECTION.east));
  ...
  it("keeps a carried bowling ball when the forward throw path is blocked", () => {
    ...
    expect(session.state.engine.inventory.tools).toEqual([MS_TILE.BowlingBall_Still]);
    expect(session.state.internal.portableTools.portableItems).toContainEqual(
      expect.objectContaining({
        family: "bowling-ball",
        tileId: MS_TILE.BowlingBall_Still,
        state: { mode: "carried" },
  ...
  it("primes a carried bowling ball for a still drop when Chip picks up another portable item", () => {
    ...
    cells[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east);
    cells[pickupPos]!.top.id = MS_TILE.Hook;
  ...
  it("activates a still bowling ball from map state when it starts on force floor", () => {
    ...
    expect(bowlingBall).toBeTruthy();
    expect(attachedPortable?.family).toBe("bowling-ball");
    expect(session.state.internal.portableTools.portableItems.find((item) => item.state.mode === "map" && item.tileId === MS_TILE.BowlingBall_Still)).toBeUndefined();
  });

  it("reverts a blocked moving bowling ball to still map state while preserving local inventory", () => {
    const cells = createEmptyCells();
    const chipPos = pos(1, 1);
    const keyPos = pos(3, 1);
    const wallPos = pos(4, 1);
    cells[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east);
    cells[keyPos]!.top.id = MS_TILE.Key_Red;
    cells[wallPos]!.top.id = MS_TILE.Wall;
    ...
    const reverted = session.state.internal.portableTools.portableItems.find(
      (item) => item.state.mode === "map" && item.tileId === MS_TILE.BowlingBall_Still && item.state.pos === keyPos && item.state.z === 1,
    );

    expect(session.state.engine.map.cells[keyPos]?.top.id).toBe(MS_TILE.BowlingBall_Still);
    expect(session.state.internal.creatures.find((creature) => creature.id === MS_TILE.BowlingBall && !creature.hidden)).toBeUndefined();
  ...
  it("denies Chip entry when chasing a moving bowling ball from behind", () => {
    const cells = createEmptyCells();
    const chipPos = pos(1, 1);
    const bowlingBallPos = pos(2, 1);
    const wallPos = pos(3, 1);
    cells[chipPos]!.top.id = msCreatureTile(MS_TILE.Chip, MS_DIRECTION.east);
    cells[bowlingBallPos]!.top.id = msCreatureTile(MS_TILE.BowlingBall, MS_DIRECTION.east);
    cells[wallPos]!.top.id = MS_TILE.Wall;
