...
describe("ms actorCollections", () => {
  ...
  it("does not let ice blocks collect chips or inventory items", () => {
    ...
    expect(collectMsActorTile(MS_TILE.IceBlock, inventory, MS_TILE.ICChip)).toMatchObject({
      collected: false,
      collectedChip: false,
    });
    expect(collectMsActorTile(MS_TILE.IceBlock, inventory, MS_TILE.Key_Red)).toMatchObject({
      collected: false,
      collectedChip: false,
      slot: null,
      index: null,
