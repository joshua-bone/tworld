...
describe("HybridCC v0 presentation-only actor motion", () => {
  it("interpolates ordinary movement across four 20 Hz presentation samples", () => {
    ...
    const tracks = advanceHybridCcV0MotionTracks(level, snapshot(actor(0), cells), snapshot(actor(1), cells), 2, new Map());

    expect(hybridCcV0ActorMotion(tracks, 1, 2)).toEqual({ frame: 0, moving: 8 });
    expect(hybridCcV0ActorMotion(tracks, 1, 3)).toEqual({ frame: 1, moving: 6 });
    expect(hybridCcV0ActorMotion(tracks, 1, 4)).toEqual({ frame: 2, moving: 4 });
    expect(hybridCcV0ActorMotion(tracks, 1, 5)).toEqual({ frame: 3, moving: 2 });
    expect(hybridCcV0ActorMotion(tracks, 1, 6)).toEqual({ frame: 0, moving: 0 });
  });

  it("uses the one-logic-step fast profile for ice, force floors, and teleports", () => {
    for (const terrain of [9, 10, 11, 12]) {
      ...
      const tracks = advanceHybridCcV0MotionTracks(
        level,
        snapshot(actor(0), previousCells),
        snapshot(actor(1), currentCells),
        2,
        new Map(),
      );
  ...
  it("uses direction-relative movement for a long teleport instead of spanning the map", () => {
    ...
    const tracks = advanceHybridCcV0MotionTracks(level, snapshot(actor(0), cells), snapshot(actor(3), cells), 2, new Map());
