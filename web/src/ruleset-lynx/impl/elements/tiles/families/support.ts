...
function handleLynxSupportBehavior(
  ...
  if (isLynxSupportingWallTile(context.tileId)) {
...
export function createLynxSupportTileBehavior(
  ...
  const hasSupportBehavior =
    ...
    isLynxSupportingWallTile(tileId);
  if (!hasSupportBehavior) {
    return undefined;
  }
  return createTileBehavior({
    "probe-support": (context) => {
      handleLynxSupportBehavior(policy, context as LynxTileSupportBehaviorContext);
    },
  });
}
