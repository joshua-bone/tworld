    ...
    async () => {
      ...
      if (useDebugTrace) {
        ...
        const actual = usedWindowedActual
          ? await (
              candidate as LynxGameEngineAdapter & {
                runReplayTraceDebugWindow: (
                  request: Parameters<LynxGameEngineAdapter["runReplayTraceDebug"]>[0],
                  replay: Parameters<LynxGameEngineAdapter["runReplayTraceDebug"]>[1],
                  maxTicks: number,
                  windowStart: number,
                  windowEndExclusive: number,
                ) => Promise<Awaited<ReturnType<LynxGameEngineAdapter["runReplayTraceDebug"]>>>;
              }
            ).runReplayTraceDebugWindow(
              scenario!.request,
              scenario!.replay,
